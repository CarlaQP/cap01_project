...
ggplot() +
  ...
  geom_sf_text(data = data_sf, 
               ...
               nudge_x= case_when(data_sf$cofradia=="Andratx" ~-0.21,
                                  ...
                                  data_sf$cofradia=="Ciutadella"~-0.25,
...
ggsave("D:/BASES DE DATOS/CAPITULO_01/graficos_cap01/mapa_entrevistas.png", dpi = 310, width = width_in_inches, height = height_in_inches, units = "in", limitsize = FALSE)
...
shapefile_path <- "D:/BASES DE DATOS/01_GIS/EspaciosAGE.shp" 
...
ggplot() +
  ...
  geom_sf(data = osm_data_coastline$osm_lines, aes(geometry = geometry), 
          color = 'black') +
  # Superposición de la shapefile (con estilo personalizado)
  geom_sf(data = shapefile_data, fill = "red", color = "red", linetype = "dashed", size = 0.7) +
  ...
  geom_sf_text(data = data_sf, 
               ...
               nudge_y= case_when(data_sf$cofradia=="Santanyí"~ -0.02,
                                  ...
                                  TRUE~0),
               nudge_x= case_when(data_sf$cofradia=="Andratx" ~-0.21,
                                  data_sf$cofradia=="Colònia de Sant Jordi"~-0.40,
                                  data_sf$cofradia=="Sant Antoni"~-0.25,
                                  data_sf$cofradia=="Sóller"~-0.17,
                                  data_sf$cofradia=="Ciutadella"~-0.20,
                                  data_sf$cofradia=="Eivissa"~0.07,
                                  data_sf$cofradia=="Formentera"~0.07,
                                  data_sf$cofradia=="Palma"~-0.07,
                                  data_sf$cofradia=="Cala Ratjada"~0.07,
